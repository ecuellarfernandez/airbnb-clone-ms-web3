...
class PaymentFailedHandler(EventHandlerInterface):
    ...
    def execute(self, event_value: dict):
        ...
        audit_log = AuditLog(
            action=UPDATE,
            user_id=user_id,
            description=f"Payment failed for reservation {reservation_id} with amount {amount}",
        ...
        print(f"[EVENT HANDLER] Processed PAYMENT_FAILED for payment_id: {payment_id}, "
